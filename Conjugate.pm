...
# Regular expressions for endings
%endg = %{verbify( q"
      o   [aei]s  [aei] [eaio]mos [ae]?is [ae]m,
...
%verb = (
     "ter"=>verbify( q{ 
       ...
       tivesse tivesses tivessemos tivessem,
...
habituar garantir iludir imitir infundir inquirir insistir infringir
inflingir impingir insurgir intermitir irromper };

# Why use DATA filehandles that are 'closed' when one needs them?
goto ALL_THE_WAY;
# $Lingua::PT::vl is  initialised 
 I_M_BACK:  
...
# HERE : Take out all these needless arguments.
sub soft_g {
    my ( $w , $root, $edg , $p , $t ) = @_ ;
...
#                print "Cpres rule $y ($vy,$cy,$ey) <$endg{cpres}->[0]> \n";
                ...
                $w = "$tg{cpres}$y$reg{$edg}->{cpres}->[$p-1]";
            }                
...
#                        print "-$endg{cpres}->[$p-1]-$y-$1-$2-$3\n";
            ...
            $w = "$tg{reg}$root$reg{$edg}->{$t}->[$p-1]"   if 
...
#        print "found for model : >>$1,$2<<\n";
    ...
    if($vlist =~ /  defectivos([123])?\s* ( = [^\:\=]*) \b$v \s*
       ([^\s=:]|\Z)  /x  
       ){
