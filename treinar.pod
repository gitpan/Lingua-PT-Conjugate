...
=head2 ACCENTUATION uses iso 8859-1 characters when the -i option is
passed. Entering a single "i" on a line will toggle iso-accentuation.

=head2 FIXING THE TENSE is done by entering line with a "f" followed
by a tense, in abbreviated form. A single "f" on a line will either
fix the tense to the current tense, or toggle off fixed-tense-mode.
